  ...
  <div class="container mx-auto my-10 py-10 px-4">
    ...
    @if(isset($notLoggedIn) && $notLoggedIn)
      {{-- <div class="text-center text-gray-500">
        <p class="mb-4">Anda belum login, silakan login terlebih dahulu.</p>
        <a href="{{ route('login') }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">
          Login
        </a>
      </div> --}}
      @if ($items->count())
        ...
        <div class="text-right mt-6 flex flex-wrap justify-end gap-2">
          ...
          <a href="{{ route('login') }}" class="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-600">
    ...
    @elseif ($items->count())
        ...
        <tbody>
          ...
          @foreach ($items as $item)
            ...
            <tr>
              <td class="border px-4 py-2">
                @if($item->gambar)
                  <img src="data:image/png;base64,{{ base64_encode($item->gambar) }}" width="60" alt={{ $item->nama }} class="object-contain" />
                @else
                  <span class="text-gray-400">Tidak ada gambar</span>
                @endif
              </td>
      ...
      <div class="text-right mt-6 flex flex-wrap justify-end gap-2">
        ...
        <a href="{{ route('checkout.form') }}" class="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-600">
          Checkout
        </a>
      </div>
