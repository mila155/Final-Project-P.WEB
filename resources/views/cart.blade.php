...
<body class="flex flex-col min-h-screen">
  ...
  <div class="container mx-auto my-10 py-10 px-4">
    ...
    <table class="min-w-full border border-gray-300 text-sm">
        ...
        @foreach ($items as $item)
          ...
          <tr>
            <td class="border px-4 py-2">
              @if($item->gambar)
                {{-- Asumsi gambar sudah dalam bentuk base64 atau file path --}}
            ...
            <td class="border px-4 py-2">
              <form action="{{ route('cart.update', $item->id) }}" method="POST" class="flex items-center gap-2">
                ...
                <button type="submit" class="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600">Edit</button>
              </form>
            </td>
      ...
      <tfoot>
        <tr>
          <th colspan="4" class="text-right border px-4 py-2 font-semibold">Total Belanja:</th>
          <th class="border px-4 py-2 font-semibold">Rp {{ number_format($grandTotal, 0, ',', '.') }}</th>
    ...
    <div class="text-right mt-6 flex flex-wrap justify-end gap-2">
      <a href="{{ route('product') }}" class="bg-gray-300 text-gray-800 py-2 px-4 rounded hover:bg-gray-400">Lanjut Belanja</a>
      <a href="{{ route('checkout.index') }}" class="bg-green-500 text-white py-2 px-4 rounded hover:bg-green-600">Checkout</a>
    </div>

    @else
      <div class="text-center text-gray-500">
        <p class="mb-4">Keranjang kamu masih kosong, yuk belanja dulu!</p>
        <a href="{{ route('product') }}" class="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">Kembali ke Produk</a>
      </div>
    @endif
  </div>
